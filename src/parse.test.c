...
TEST(shttp_string_number, normal) {
  char num[] = "1234";
  shttp_u16 n;
  assert_int_equal(sizeof(num) - 1, shttp_string_number(&n, num, sizeof(num)));
...
TEST(shttp_parse_request_start_line, normal) {
  ...
  assert_int_equal(sizeof(msg) - 1,
                   shttp_parse_request_start_line(&req, msg, sizeof(msg)));
  assert_int_equal(SHTTP_METHOD_GET, req.method);
  assert_string_equal("/", req.path);
  assert_int_equal(SHTTP_VERSION_1_1, req.version);
}
...
TEST(shttp_parse_request_header, normal) {
  ...
  char msg[] = "Host: google.com\r\n\r\n";
  assert_int_equal(sizeof(msg) - 1,
                   shttp_parse_request_header(&req, msg, sizeof(msg)));
...
int main(void) {
  const struct CMUnitTest tests[] = {
    ADD(shttp_string_number, normal),
    ADD(shttp_token_cpy, space_end),
    ADD(shttp_token_cpy, null_end),
    ADD(shttp_token_cpy, newline_end),
    ADD(shttp_token_cpy, carriage_end),
    ADD(shttp_token_cmp, space_end),
    ADD(shttp_token_cmp, null_end),
    ADD(shttp_token_cmp, newline_end),
    ADD(shttp_token_cmp, carriage_end),
