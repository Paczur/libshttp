#ifndef SHTTP_KEEP_ALIVE_H
#define SHTTP_KEEP_ALIVE_H
...
shttp_keep_alive_signal_handler(shttp_socket sock[static 1]);
 ...
 * SPACE_EXPECTED
 */
SHTTP_UNUSED_RESULT shttp_status shttp_keep_alive_next(
  shttp_request req[static 1], shttp_mut_slice buff[static 1],
...
SHTTP_UNUSED_RESULT shttp_status shttp_keep_alive_next_nblk(
  shttp_request req[static 1], shttp_mut_slice buff[static 1],
  shttp_socket sock[static 1]);
...
SHTTP_UNUSED_RESULT shttp_status shttp_keep_alive_send(
  shttp_mut_slice buff[static 1], const shttp_response res[static 1]);
...
SHTTP_UNUSED_RESULT shttp_status
shttp_keep_alive_close(shttp_socket sock[static 1], shttp_conn_id id);

// returns: SOCK_CREATE, SOCK_BIND, SOCK_LISTEN
SHTTP_UNUSED_RESULT shttp_status
shttp_keep_alive_init(shttp_socket sock[static 1], uint32_t port);

// returns: CONN_FD_CLOSE, SOCK_FD_CLOSE
SHTTP_UNUSED_RESULT shttp_status
shttp_keep_alive_deinit(shttp_socket sock[static 1], bool force);
#endif
